...
<?= $this->section('main') ?>
    ...
    <div class="modal fade" id="add-new-school-modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form id="add-new-tvet-school-form" class="form-validation"> 
    ...
    <!-- edit modal form -->
    <div class="modal fade" id="edit-tvet-school-modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form id="update-tvet-school-form"  class="form-validation"> 
                    <div class="modal-body ">
...
<?= $this->section('pageScript') ?>
    ...
    <script>
        $(document).ready(function() {
                            ...
                            action     = '\
                                    <div class="btn-group">\
                                        ...
                                        <div class="dropdown-menu">\
                                            <a data-id="'+row.id+'" class="dropdown-item text-warning" href="#" id="edit-tvet-school-button">\
                                                <i class="mdi mdi-grease-pencil"></i>\
                                                <span class="nav-text">Edit Details</span>\
                                            </a>\
                                            <a data-id="'+row.id+'" class="dropdown-item text-danger" href="#" id="delete-tvet-school-button">\
                                                <i class="mdi mdi-grease-pencil"></i>\
            ...
            $(document).on('submit', '#add-new-tvet-school-form', function(e){ 
                e.preventDefault();    
                var _this = $(this)
                $.ajax({
                    url     : 'tvetschool/insert',
                    method  : "post", 
                    data    : $("#add-new-tvet-school-form").serialize(),
                    dataType: "json", 
                    success : function (data) {  
                        if(data.response){ 
                            ...
                            table.ajax.reload()
                            $("#add-new-tvet-school-form")[0].reset()
            ...
            $(document).on('click', '#edit-tvet-school-button', function(e){ 
                e.preventDefault(); 
                $('#edit-tvet-school-modal').modal('show') 

                var id = $(this).data('id')
                $.ajax({
                    url     : 'tvetschool/get/' + id,
                    method  : "get",
                    dataType: "json", 
                    success : function (data) {  
                        console.info(data)
                        $('#update-tvet-school-form input[name="id"]').val(data.id)
                        $('#update-tvet-school-form input[name="school_name"]').val(data.school_name)
                        $('#update-tvet-school-form input[name="abbreviation"]').val(data.abbreviation)
                        $('#update-tvet-school-form input[name="address"]').val(data.address)
                        $('#update-tvet-school-form select[name="manager"]').val(data.manager)
                    },
                    error   : function (xhr, status, error) { 
                        console.info(xhr.responseText);
                    }
            ...
            $(document).on('submit', '#update-tvet-school-form', function(e){ 
                e.preventDefault();    
                var _this = $(this)
                $.ajax({
                    url     : 'tvetschool/update',
                    method  : "post", 
                    data    : $("#update-tvet-school-form").serialize(),
                    dataType: "json", 
                    success : function (data) { 
                        if(data.response){ 
                            Swal.fire({
                            ...
                            table.ajax.reload() 
                            $('#edit-tvet-school-modal').modal('hide')
                        }else{ 
                            Swal.fire({
                                title: "Update Error!",
                                text : data.message,
            ...
            $(document).on('click', '#delete-tvet-school-button', function(e){ 
                e.preventDefault();  
